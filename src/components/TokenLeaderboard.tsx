    ...
    <div className="space-y-6">
      ...
      <div className="hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-fr">
        {topTokens.map((token, idx) => (
          ...
          >
            <Card 
              className={`${getPadding(idx)} h-full relative overflow-hidden transition-all duration-500 border-2 bg-gradient-to-br ${getHeatColor(token.volume_ton, maxVolume)}`}
            >
              <div 
      ...
      <div className="sm:hidden space-y-3">
        {topTokens.map((token, idx) => (
          ...
          >
            <Card 
              className={`relative overflow-hidden transition-all duration-500 border-2 bg-gradient-to-r ${getHeatColor(token.volume_ton, maxVolume)} ${
                idx === 0 ? 'p-6' : 'p-4'
              } group`}
            >
